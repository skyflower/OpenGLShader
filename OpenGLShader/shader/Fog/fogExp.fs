...
float CalcExpFog(float dist)
{
	float factor = exp(-dist * v_Attention);
	return clamp(factor, 0.0, 1.0);
}
float CalcExpXFog(float dist)
{
	float factor = exp(-pow(dist * v_Attention, v_FogGradient));
	return clamp(factor, 0.0, 1.0);
}
...
void main()
{
	...
	float fogAlpha = CalcExpXFog(abs(v_eyePos.z / v_eyePos.w));
	fogAlpha = 1 - fogAlpha;
